...
long	from_v, to_v;	/* Mathomatic variables */
{
	...
	if (en < 0 || en >= n_equations) {
#if	DEBUG
		error_bug("Invalid equation number given to rename_var_in_es().");
#else
		return;
#endif
	}
 ...
 * If no equation number or range is given, assume the current equation is wanted and
 * don't skip anything.
 *
 * Return true if successful,
...
use_current:
				*jp = *ip = cur_equation;
#if	1
				return(!empty_equation_space(cur_equation));	/* don't display error message */
#else
				return(!current_not_defined());		/* display an error message if error */
#endif
			}
		}
		(cp)++;
...
/*
 * Skip over comma and space characters.
 */
char *
skip_comma_space(cp)
char	*cp;	/* character pointer */
