 ...
 *
 * Return true if something was approximated.
 */
int
...
int		*np;
{
	...
	rv = subst_constants(equation, np);
	approximate_roots = true;
	simp_loop(equation, np);
	if (factor_imaginary(equation, np)) {
		rv = true;
	}
	approximate_roots = false;
	return rv;
...
#else
#if	_XOPEN_SOURCE >= 600 || _ISOC99_SOURCE || __USE_ISOC99 || MINGW
