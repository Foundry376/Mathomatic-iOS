...
#if	!LIBRARY
{	"calculate",	NULL,			calculate_cmd,		"[\"repeat\"] [\"factor\"] [variable number-of-iterations]",	"Temporarily plug in values for variables and approximate." },
#endif
...
{	"factor",	NULL,			factor_cmd,		"[\"number\" [integers]] or [equation-number-range] [variables]",	"Factor variables in equation spaces or factor given integers." },
{	"fraction",	NULL,			fraction_cmd,		"[equation-number-range]",				"Convert to a single simple fraction, like \"simplify fraction\" does." },
...
{	"solve",	NULL,			solve_cmd,		"[\"verify\"] [\"for\"] variable or \"0\"",		"Solve the current equation for a variable or for zero." },
...
next_argument:
		...
		if (strncasecmp(cp, "copyright", cp1 - cp) == 0) {
			P("Copyright and License Information for Mathomatic");
			P("------------------------------------------------");
			fprintf(gfp, "%s\n", license_string);
			return true;
		}
		if (strncasecmp(cp, "bugs", cp1 - cp) == 0) {
			P("Report bugs on the Ubuntu Launchpad website:");
			P("http://launchpad.net/mathomatic\n");

			P("Please include version information from the version command:\n");
		...
		if (is_all(cp)) {
			...
			fprintf(gfp, "End of list; total of %d different commands.\n", ARR_CNT(com_list));
			return true;
		}
