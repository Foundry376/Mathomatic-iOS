...
#define	VERSION		"1.3"	/* The current version number of this primes program. */
...
#elif LDBL_DIG < 18
#warning "long double number of digits precision is less than 18."
#endif

#if	USE_DOUBLES
typedef double double_type;
#warning "Using only double floats instead of long double floats."
#else
...
/* process command line options: */
	while ((i = getopt(argc, argv, "c:thup:m:")) >= 0) {
		switch (i) {
		case 'c':
...
get2:
		fprintf(stderr, "Enter number of%s%s primes to output (0 to end)",
		    pal_flag ? " palindromic" : "", twin_flag ? " twin" : "");
#if	USE_DOUBLES
...
usage(int ev)
{
	printf("\nPrime number generator version %s\n", VERSION);
